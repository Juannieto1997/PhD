%% p_TimeFrequencySingle
% Code to run the time frquency analisis on a single channel to adjust the
% parameters of thte functions, same functionality as p_TimeFrequency
% without the for. 
...
% Calculate limits
pv_Limits = [min(min(m_MorseWT)) max(max(m_MorseWT))]*0.01;
