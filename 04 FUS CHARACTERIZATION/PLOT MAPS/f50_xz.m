...
%% Paramètres
cd('C:\Users\Juan\Downloads\RE__Question_about_Hydrophone_+_meeting\temp');
%fileformat = 'scanTestZ%03dX%03d.txt';
fileformat = 'scanZ%03dY%03d.txt';
focal = 50;

% nombre de lignes et colonnes acquises
n_row = 60; % <-- deuxieme variable
n_col = 40; % <-- premiere variable
...
for i=0:1:n_row-1
    for j=0:1:n_col-1
      ...
      pmin_array(n,:) = [pmin,z,x];
      pmax_array(n,:) = [pmax,z,x];
      prms_array(n,:) = [prms,z,x];
      n = n+1;
    end
    100*n/(n_row*n_col)
    z = 0;
...
set(gca, 'FontSize', fsz); %<- Set properties
set(gcf,'InvertHardcopy','on');
[cv,ch]=contourf(z,x,max_array,80);
set(ch,'edgecolor','none');
axis equal
axis([-floor(n_col/2)*dz+z0 floor(n_col/2)*dz+z0 -floor(n_row/2)*dx+x0 floor(n_row/2)*dx+x0])
axisLabel = get(gca,'YTickLabel');
set(gca,'YTickLabel',axisLabel(end:-1:1));
% set(gca, 'YDir', 'reverse'); %<- Set properties
...
set(gca, 'FontSize', fsz); %<- Set properties
set(gcf,'InvertHardcopy','on');
[cv,ch]=contourf(z,x,-min_array,80);
set(ch,'edgecolor','none');
axis equal
axis([-floor(n_col/2)*dz+z0 floor(n_col/2)*dz+z0 -floor(n_row/2)*dx+x0 floor(n_row/2)*dx+x0])
axisLabel = get(gca,'YTickLabel');
...
set(gca, 'FontSize', fsz); %<- Set properties
set(gcf,'InvertHardcopy','on');
[cv,ch]=contourf(z,x,rms_array,80);
set(ch,'edgecolor','none');
axis equal
axis([-floor(n_col/2)*dz+z0 floor(n_col/2)*dz+z0 -floor(n_row/2)*dx+x0 floor(n_row/2)*dx+x0])
axisLabel = get(gca,'YTickLabel');
...
title(' ')
c = colorbar;
ylabel(c,'Pressure, \itp_{RMS} \rm(MPa)')
% set(gca, 'YDir', 'reverse'); %<- Set properties
...
set(gcf, 'Position', [pos(1) pos(2) (width) (height)]);
[X,Y] = meshgrid(y,x)
[M,c] = contourf(z,x,max_array_dB,[-12 -9 -6 -3],'Fill','on')
