...
str_SaveName = 'PressureSimulation2025603Free.mat'; % name of the file to save 
...
v_Inte = m_Data(:,9);% Power at the transducer 
v_Pres = zeros(1,length(v_Freq));
v_peakRMS = zeros(1,length(v_Freq));
v_Psapa = zeros(1,length(v_Freq));
v_Psar = zeros(1,length(v_Freq));
v_Isapa = zeros(1,length(v_Freq));
%% Parametes for focal Spot 
s_dBFocal = -3;
%% run the simulation for each value
for idx = 114:length(v_Freq)
    ...
    [~,idxmax] = min(abs(z_vec_AS - 0.015));
    p_mag_x_profile= p_mag(idxmax,:);
    p_mag_x_profile=[p_mag_x_profile(length(p_mag_x_profile):-1:1) p_mag_x_profile];
    v_Pres(idx) = max(p_mag_x_profile);
    v_peakRMS(idx) = v_Pres(idx) /sqrt(2);
    s_FocSp = v_Pres(idx)*10^(s_dBFocal/20);
    v_cut =f_zeroCross (p_mag_x_profile-s_FocSp);
    s_Psapa = sum(p_mag_x_profile(v_cut(1):v_cut(2)))/abs(diff(v_cut));
    v_Psapa(idx) = s_Psapa;
    v_Psar(idx) = s_Psapa/(sqrt(2));
    v_I = ((p_mag_x_profile).^2)/(2*s_Z);
    s_I = sum(v_I(v_cut(1):v_cut(2)));
    s_PixSize = (mean(diff(x_vec_AS)))^2;% Pixel size in m^2
    v_Isapa(idx) = s_I/((v_cut(2)-v_cut(1))*s_PixSize);
...
save(str_SaveName,'v_Freq','v_peakRMS','v_Driv','v_Inte','v_Pres','v_Psapa','v_Psar','v_Isapa','sensor_data','kgrid','tx_freq','Nz','Nx')