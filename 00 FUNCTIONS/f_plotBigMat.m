function f_plotBigMat(m_Data,c_Chann,str_slice,v_Lims);
...
for i=1:120
    ...
    v_Data = v_FilData(1:20:end);                                          % Subsampling to improvee plot speed reduce memory use
    %v_Pks = floor((stru_Pks(i).PksIndx)./20);
    %v_Aux = 1:numel(v_Data);

    plot(a_plots(s_IndexPlot),v_Data,'Color',[0 16 199]/255,'LineWidth',1);
