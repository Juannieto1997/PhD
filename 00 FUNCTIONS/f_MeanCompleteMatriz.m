function f_MeanCompleteMatriz (m_Data,c_channels,str_savepath,str_slice,v_Lims)
%% Create figure
f= figure();
for i = 1:144
...
for i=1:numel(v_Ax2Remove)
    ...
    cll_ChannOrder = {'D1','E1','F1','G1','H1','J1',... % From datasheet
    ...
    for idxChan = 1:length(c_channels)
        str_currChannel = c_channels{idxChan};
        ...
        plot(a_plots(s_CorresPlot),v_time,mean(m_CurrData)-mean(mean(m_CurrData)),'Color',[95 15 64]/255,'LineWidth',2);
        %plot(a_plots(s_CorresPlot),v_time,(m_CurrData(3,:)),'Color',[0 16 199]/255,'LineWidth',3);
        %plot(a_plots(s_CorresPlot),v_time,m_CurrData,'Color',[95 15 64]/255,'LineWidth',2);
        catch
            continue
        end
